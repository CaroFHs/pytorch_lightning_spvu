...
from .modules.module_attention import ModifiedSpatialTransformer
...
class SeD_P(nn.Module):
    ...
    ## SeFB1
        ...
        upscale = math.ceil(64 / semantic_size) # 计算放大比例
    ...
    def forward(self, input, semantic):
        ...
        se = self.att1(semantic, input) # cross attention1
        input = self.lrelu(self.conv11(torch.cat([input, se], dim=1)))
        
        input = self.conv2(input)
        se = self.att2(semantic, input) # cross attention2
        input = self.lrelu(self.conv21(torch.cat([input, se], dim=1)))
        
        input = self.conv3(input)
        se = self.att3(semantic, input) # cross attention3
...
if __name__ == '__main__':
    ...
    # model_d = SeD_P(input_nc=1)
    model_d = SeD_U(num_in_ch=1)
    model_se = CLIP_Semantic_extractor()
    x = torch.randn(4,1,128,128)
    x_ex = model_se(x)
    y = model_d(x, x_ex)
    print(x.size())    # (b,c,h,w)
    print(x_ex.size()) # (b,1024,h_ex,w_ex), h_ex=h/16, w_ex=w/16
    print(y.size())    # SeD_P:(b,c,(h-h_ex)/8,(w-w_ex)/8); SeD_U:(b,c,h,w)
    print(y)
