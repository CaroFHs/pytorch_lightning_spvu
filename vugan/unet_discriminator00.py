...
class Unet_Discriminator(nn.Module):
    ...
    def __init__(self, 
                 ...
                 D_init='ortho', # 权重初始化方法，默认采用 'ortho'（正交初始化）
        ...
        # last_layer = nn.Sequential(*[nn.Conv2d(self.ch * self.out_channel_multiplier, 1, kernel_size=1), nn.Tanh()]) # 改为tanh
        last_layer = nn.Conv2d(self.ch * self.out_channel_multiplier, 1, kernel_size=1)
...
#############################################################################################
    ...
    def init_weights(self, init_type='kaiming'):
        ...
        for m in self.modules():
            if isinstance(m, (nn.Conv2d, nn.ConvTranspose2d, nn.Linear, nn.Embedding)):
                if init_type == 'ortho':
                    ...
                    init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu') # leaky_relu or relu
                else:
        ...
        print('Param count for model initialized parameters: %d' % self.param_count)
...
if __name__ == '_main__':
    d_model = Unet_DiscriminatorGenerator(input_c=1)
    print(d_model)
