...
class Conv2d_cd(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
        ...
        self.theta = theta # 自定义权重，用于计算权重
    
    def get_weight(self):
        ''' 获取、处理卷积层的权重 '''
        conv_weight = self.conv.weight # 获取卷积层权重
        conv_shape = conv_weight.shape # 获取卷积层权重形状(c_in, c_out, k1, k2)
        conv_weight = Rearrange('c_in c_out k1 k2 -> c_in c_out (k1 k2)')(conv_weight) # 变换形状方便处理
        # conv_weight_cd = torch.cuda.FloatTensor(conv_shape[0], conv_shape[1], 3 * 3).fill_(0)
        conv_weight_cd = torch.zeros(conv_shape[0], conv_shape[1], 3 * 3, dtype=torch.float32, device='cuda') # 创建中心差分卷积权重
        conv_weight_cd[:, :, :] = conv_weight[:, :, :] # 中心差分卷积权重赋值

        ## 对于每个输出通道，中心位置（索引为 4）的权重值被调整为原始中心位置的权重值减去所有其他位置权重值的总和。
        ## 这一步实现了中心差分的效果，即强调中心像素的权重，并相应地减少周围像素的权重
        conv_weight_cd[:, :, 4] = conv_weight[:, :, 4] - conv_weight[:, :, :].sum(2)

        ## 转换回原始卷积核的形状
        conv_weight_cd = Rearrange('c_in c_out (k1 k2) -> c_in c_out k1 k2', k1=conv_shape[2], k2=conv_shape[3])(conv_weight_cd)
        
        return conv_weight_cd, self.conv.bias # 返回权重和偏差
...
class DEC(nn.Module):
    def __init__(self, dim):
        ...
        self.conv1_1 = Conv2d_cd(dim, dim, 3, bias=True) # 中心差分卷积
        self.conv1_2 = Conv2d_hd(dim, dim, 3, bias=True) # 水平差分卷积
        self.conv1_3 = Conv2d_vd(dim, dim, 3, bias=True) # 垂直差分卷积
        self.conv1_4 = Conv2d_ad(dim, dim, 3, bias=True) # 角度差分卷积
        self.conv1_5 = nn.Conv2d(dim, dim, 3, padding=1, bias=True) # 普通卷积
