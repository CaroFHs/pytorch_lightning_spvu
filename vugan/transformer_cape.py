...
from models.vugan.vugan_select import get_norm_layer, get_activ_layer
...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.ffn   = PositionWiseFFN(features, ffn_features, activ) # 前馈神经网络
...
#         '''
#         初始化，输入：
#         features: 输出特征的维度； height: 输入图像的高度；
#         width: 输入图像的宽度； **kwargs: 其他可选参数，传递给父类的构造函数
#         '''
#         super().__init__(**kwargs) # 调用父类
...
###################### 改01，将CAPE合入patch_emb部分 ###########################
class ViTInput(nn.Module):
    ''' VIT输入（改） '''
    def __init__(
        self, input_features, image_shape,
        **kwargs
    ):
        '''
        初始化，输入：
        input_features：    输入图像块的特征数； image_shape[0] = 384
        embed_features：    位置嵌入的特征数； 384 (定义于训练文件中**kwargs)
        features：          输出特征数； 384 (定义于训练文件中**kwargs)
        height 和 width：   图像的高度和宽度； 16,16
        **kwargs：          其他额外参数；
        '''
        # 输入：[b, 384, 16, 16]
        super().__init__()
        self.image_shape = image_shape # [384, h, w]

    #### content-aware positional embedding
        # [b, 384, h, w]
        self.new_ps = nn.Conv2d(input_features , input_features , (1,1))

        ########################################################
        # Add new parameters if necessary
        self.new_param = nn.Parameter(torch.randn(
                    2, input_features, self.image_shape[1], self.image_shape[2]))
        ########################################################
    ...
    def forward(self, x):
        ...
        ## CAPE
        # [1, 384, 32, 32] → [1, 384, 16, 16]
        pos_img = self.averagepooling(x)
        pos_img = self.new_ps(pos_img) 


        # pos_img → (N, C, H * W) = [1, 384, 256]
        pos_img = pos_img.view(*pos_img.shape[:2], -1)

        # pos_img → (N, H*W, C) → (N, L, C) = [1, 256, 384]
...
#         '''
#         初始化，输入：
#         input_features：    输入图像块的特征数；f_dim
#         embed_features：    位置嵌入的特征数；
#         features：          输出特征数； 
#         height 和 width：   图像的高度和宽度；16
#         **kwargs：          其他额外参数；16
#         '''
#         super().__init__(**kwargs) # 调用父类
#         self._height   = height
...
#         x, y   = torch.meshgrid(x, y) # 创建一个网格，将 x 和 y 坐标结合起来，生成所有像素的位置
...
class PixelwiseViT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs：          其他额外参数。
        ...
        self.image_shape = image_shape # [384, 16, 16]

    ###################### 原self.trans_input ##############################

        # self.trans_input = ViTInput_0(
    ...
    ###################### 改03，self.trans_input替换为 ##############################
        self.trans_input = ViTInput(image_shape[0], image_shape) 
        # 创建vitinput实例，将输入图像块进行嵌入和位置编码
        # ViTInput(输入图像块的特征数，位置嵌入的特征数，输出特征数，图像高度，图像宽度)
    ################################################################################
    ...
    ############# 改02，不做预处理，将UNet的输出直接作为patch_emb的输入 ##################
    def forward(self, x):
        # x : (N, C, H, W) = [b, 384, 32, 32]
        '''
        改为不做预处理
    #### pre-transforms
    ...
    #### patch embedding & pos embedding
        # y : (N, L, features) = [b, 256, 384]
        # y = self.trans_input(itokens)
        y = self.trans_input(x)

    #### transformer encoder
        y = self.encoder(y) # [b, 256, 384]

    #### linear layer
        # otokens : (N, L, C) = [b, 256, 384]
        ...
        height, weight = self.image_shape[1:]
        # print(height,weight)

        # result : (N, C, H, W) = [1, 384, 16, 16]

        # ## *otokens.shape[:2] = b, 384; *self.image_shape[1:] = vit输入特征的高和宽
        # result = otokens.view(*otokens.shape[:2], *self.image_shape[1:])
        result = otokens.view(*otokens.shape[:2], 16, 16) # 

        # [1, 384, 16, 16] → [1, 384, 32, 32]，双线性插值
        result = F.interpolate(result, mode='bilinear',size= self.image_shape[1])
        ############################################################
